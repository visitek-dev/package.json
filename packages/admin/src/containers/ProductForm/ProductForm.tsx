...
const CREATE_PRODUCT = gql`
  mutation createProduct($product: AddProductInput!, $file: [Upload!]!) {
...
const AddProduct: React.FC<Props> = props => {
    ...
    const options = data?.categories.map(item => item);
    ...
    const children = tag[0]?.children.map(item=> item);
    const image = file?.map(res=> res.name );
    const onSubmit = data => {
        const product = {
            sku: data.sku,
            name: data.name,
        ...
        createProduct({
            variables: { product: product,file: file},
        });
        closeDrawer();
    };
    return (
                        ...
                        <Col lg={8}>
                            <DrawerBox>
                                <FormFields>
                                    ...
                                    <Input type="text" inputRef={register({required: true})} name="sku"/>
                                </FormFields>
                                <FormFields>
                                ...
                                <FormFields>
                                <FormLabel>Category Children</FormLabel>
                                <Select
                                    options={children}
                                    labelKey="name"
                                    valueKey="name"
